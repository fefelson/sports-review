...
from ..events import EVT_PointSpread
...
class PointSpreadFrame(wx.Frame):

    def __init__(self, parent, teamIds=[], *args, **kwargs):
        super().__init__(parent, size=(900,800), *args, **kwargs)
        self.Bind(EVT_PointSpread, self.setPanel)
    ...
    def setPanel(self, evt):
        info = evt.GetValue()
        hA = "home"
        spread = "+"+str(float(info["spread"])) if float(info["spread"]) > 0 else float(info["spread"])
        self.title.SetLabel("{}  {}".format(hA.capitalize(), spread))
        self.chart.setPanel(info)

        if info["exactMatch"]:
            self.hist1.setPanel(info["exactMatch"][hA])
            self.writ1.setPanel(info["exactMatch"])
            self.writ1.title.SetLabel("exact match")

        self.hist2.setPanel(info["pointSpread"][hA])
        self.writ2.setPanel(info["pointSpread"])
        self.writ2.title.SetLabel("point spread match")
        self.panel.Layout()
